...
A multi-agent orchestration system built as a Telegram bot in Zig 0.14+. Agents have tools (bash execution) and skills (markdown knowledge files). Teams of agents collaborate on complex tasks coordinated by an orchestrator.
...
- **`agent.zig`** - `AgentDef` (with `tool_names` and `skill_names`), `AgentRuntime` implementing the **tool-use loop**: LLM call -> execute tool calls -> feed results back -> repeat until text response.

### Layer 3: Team & Orchestration
- **`team.zig`** - `TeamDef` with roles (lead/member/reviewer) mapping to agent IDs
...
### Layer 4: Application
- **`main.zig`** - Per-chat state (`ChatState` with `direct_agent` or `team_task` mode), command dispatch, working directory support via CLI arg.
